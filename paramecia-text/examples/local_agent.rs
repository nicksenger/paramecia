...
//!
//! Usage:
//!   cargo run --features=cuda --release -p paramecia-text --example=local_agent \
...
use paramecia_harness::{ParameciaConfig, get_universal_system_prompt_with_tools};
...
struct Args {
    ...
    /// Device offload mode: none, down, updown, or experts.
...
fn get_system_prompt_from_harness(
    ...
    let mut config = match ParameciaConfig::load(None) {
        Ok(c) => c,
        Err(e) => {
            eprintln!("Warning: Failed to load config, using defaults: {}", e);
            ParameciaConfig::default()
        }
    };
...
async fn main() -> anyhow::Result<()> {
    ...
    let provider = ProviderConfig {
        ...
        context_length: Some(args.context_length),
        local_kv_cache_quant: None, // Use default (Q8_0)
        local_layer_split: args
            .layer_split
            .clone()
            .or_else(|| std::env::var("PARAMECIA_LAYER_SPLIT").ok()),
        local_disable_context: None,
        ...
        tuning_output: None,          // Disabled by default
        tuning_top_k: None,
    ...
    let local_backend = LocalBackend::new(provider)?;
    ...
    // temperature=0.6, top_p=0.95, top_k=20, min_p=0.0, repeat_penalty=1.05, presence_penalty=1.0
    ...
    for turn in 0..args.max_turns {
        ...
        while let Some(event_result) = stream.next().await {
            ...
            match event {
                StreamEvent::ContextTokens { .. } => {
                    // Prefill progress event (no text output).
                }
        ...
        // No tool calls - add message and break
        messages.push(LlmMessage {
            role: Role::Assistant,
            content: if full_content.is_empty() {
                None
