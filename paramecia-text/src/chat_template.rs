...
/// Qwen-Coder chat template (pure XML tool call format).
///
/// This template uses the XML-based tool call format:
/// ```text
/// <tool_call>
///   <function=name>
...
/// </tool_call>
/// ```
pub const QWEN_CODER_CHAT_TEMPLATE: &str = r#"{%- if tools %}
    {{- '<|im_start|>system\n' }}
    {%- if messages[0]['role'] == 'system' %}
        {{- messages[0]['content'] }}
    {%- else %}
    ...
    {%- for tool in tools %}
        ...
        {{- tool | tojson }}
    {%- endfor %}
    {{- "\n</tools>\n\nFor each function call, return the call within <tool_call></tool_call> tags using this XML format:\n<tool_call>\n<function=function_name>\n<parameter=param_name>value</parameter>\n</function>\n</tool_call><|im_end|>\n" }}
{%- else %}
    {%- if messages[0]['role'] == 'system' %}
        {{- '<|im_start|>system\n' + messages[0]['content'] + '<|im_end|>\n' }}
    {%- endif %}
