...
impl ApiToolFormatHandler {
    ...
    ///
    /// Accepts both JSON-in-XML and pure XML formats within the same text.
    /// Each `<tool_call>` block is independently detected.
    #[must_use]
    pub fn parse_tool_calls_from_content(&self, content: &str) -> Vec<ToolCall> {
        let parsed = self.xml_parser.parse_any_xml(content);
