...
pub fn process_experts_indexed(
    ...
    let gate_out = gate_exps
        .read()
        .unwrap()
        .indexed_moe_forward(&hidden_3d, &indices_cpu)?;
    ...
    let activated = paramecia_nn::ops::silu(&gate_out)?.mul(&up_out)?;

    // Down projection: [n_tokens, top_k, hidden_dim]
    let down_out = down_exps
        .read()
        .unwrap()
        .indexed_moe_forward(&activated, &indices_cpu)?;
...
pub fn process_experts_tokio(
    ...
    let hidden_cpu = if !matches!(original_device, Device::Cpu) {
    ...
    // Fast exit if nothing is routed.
    if !expert_assignments
        .iter()
        .any(|(_, indices, _)| !indices.is_empty())
    {
        return hidden_flat.zeros_like();
    }

    // Process experts in parallel using Rayon
    let results: Vec<Result<(Vec<u32>, Vec<f32>)>> = expert_assignments
        .par_iter()
        .filter(|(_, token_indices, _)| !token_indices.is_empty())
...
fn process_single_expert_optimized(
    ...
    let gate = gate_exps.read().unwrap().slice_first_dim(expert_idx)?;
    let up = up_exps.read().unwrap().slice_first_dim(expert_idx)?;
    let down = down_exps.read().unwrap().slice_first_dim(expert_idx)?;
