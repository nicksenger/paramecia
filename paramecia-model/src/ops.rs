...
use paramecia_core::{DType, Result, Tensor, D};
...
/// Top-k routing directly from router logits.
///
/// Input: router_logits [batch, seq_len, n_experts] (pre-softmax).
/// Returns: (top_weights, top_indices) each of shape [batch, seq_len, topk].
///
/// This dispatches to fused backend kernels (`topk_softmax`) where available.
pub fn topk_moe_routing_from_logits(
    router_logits: &Tensor,
    topk: usize,
) -> Result<(Tensor, Tensor)> {
    let router_logits = router_logits.contiguous()?;
    let (batch_size, seq_len, n_experts) = router_logits.dims3()?;
    let logits_flat = router_logits.reshape((batch_size * seq_len, n_experts))?;
    let (top_weights, top_indices) = paramecia_core::deltanet_ops::topk_softmax(&logits_flat, topk)
        .map_err(|e| {
            e.context(format!(
                "topk_moe_routing_from_logits failed: input_shape={:?}, input_stride={:?}, input_start_offset={}, reshaped_shape={:?}, reshaped_stride={:?}, reshaped_start_offset={}, topk={}",
                router_logits.dims(),
                router_logits.stride(),
                router_logits.layout().start_offset(),
                logits_flat.dims(),
                logits_flat.stride(),
                logits_flat.layout().start_offset(),
                topk
            ))
        })?;
    let top_weights = top_weights.reshape((batch_size, seq_len, topk))?;
...
fn topk_moe_routing_optimized(routing_weights: &Tensor, topk: usize) -> Result<(Tensor, Tensor)> {
    let routing_weights = routing_weights.contiguous()?;
    ...
    let (sorted_vals, sorted_idx) = weights_flat.sort_last_dim(false).map_err(|e| {
        e.context(format!(
            "topk_moe_routing_optimized sort_last_dim failed: input_shape={:?}, input_stride={:?}, input_start_offset={}, reshaped_shape={:?}, reshaped_stride={:?}, reshaped_start_offset={}, topk={}",
            routing_weights.dims(),
            routing_weights.stride(),
            routing_weights.layout().start_offset(),
            weights_flat.dims(),
            weights_flat.stride(),
            weights_flat.layout().start_offset(),
            topk
        ))
    })?; // descending
...
pub fn group_tokens_by_expert(
    ...
    let expert_flat = expert_indices
        .flatten_all()?
        .to_dtype(DType::U32)?
        .contiguous()?;
    ...
    let (sorted_token_ids, sorted_k_ids, sorted_expert_ids) = if total_pairs > 1024 {
        ...
        let expert_cpu: Vec<u32> = expert_flat
        ...
        let sort_indices = expert_flat.arg_sort_last_dim(true).map_err(|e| {
            e.context(format!(
                "group_tokens_by_expert arg_sort_last_dim failed: expert_indices_shape={:?}, expert_indices_stride={:?}, expert_indices_start_offset={}, expert_flat_shape={:?}, expert_flat_stride={:?}, expert_flat_start_offset={}, num_experts={}",
                expert_indices.dims(),
                expert_indices.stride(),
                expert_indices.layout().start_offset(),
                expert_flat.dims(),
                expert_flat.stride(),
                expert_flat.layout().start_offset(),
                num_experts
            ))
        })?; // ascending
...
pub fn process_moe_gpu_grouped(
    ...
    gate_exps: &paramecia_core::quantized::SharedQTensor,
    up_exps: &paramecia_core::quantized::SharedQTensor,
    down_exps: &paramecia_core::quantized::SharedQTensor,
    num_experts: usize,
) -> Result<Tensor> {
    ...
    let expert_results: Vec<(usize, ExpertResult)> = expert_assignments
        ...
        .filter_map(|(expert_id, assignments)| {
            ...
            let gate = gate_exps.read().unwrap().slice_first_dim(expert_id).ok()?;
            let up = up_exps.read().unwrap().slice_first_dim(expert_id).ok()?;
            let down = down_exps.read().unwrap().slice_first_dim(expert_id).ok()?;
...
pub fn solve_lower_triangular_batched(
    ...
    if l_dims.len() != 4 || b_dims.len() != 4 {
        paramecia_core::bail!("solve_lower_triangular_batched expects 4D tensors");
    }
    ...
    if n != n2 {
        paramecia_core::bail!("L matrix must be square in last two dimensions");
    }
    ...
    let x_flat = paramecia_core::deltanet_ops::solve_i_minus_lower_triangular(&l_flat, &b_flat)?;
...
pub fn l2_normalize_scale(x: &Tensor, scale: f32, eps: f64) -> Result<Tensor> {
    ...
    paramecia_core::deltanet_ops::l2_normalize_scale(x, scale, eps)
}
...
pub fn delta_net_autoregressive_step(
    ...
    // Delegate to candle-core's fused CUDA kernel implementation
    paramecia_core::deltanet_ops::delta_net_autoregressive_step(
...
pub fn delta_net_multi_token_update(
    ...
    // Delegate to candle-core's fused CUDA kernel implementation
    paramecia_core::deltanet_ops::delta_net_multi_token_update(q, k, v, gate, beta, state)
}
...
pub fn delta_net_parallel_with_states(
    ...
    // Delegate to candle-core's fused CUDA kernel implementation
    paramecia_core::deltanet_ops::delta_net_parallel_with_states(q, k, v, gate, beta, state)
}
...
pub fn compute_draft_probs(
    ...
    paramecia_core::deltanet_ops::compute_draft_probs(
        target_logits,
        draft_logits,
        draft_tokens,
        temperature,
...
pub fn fused_swiglu(gate: &Tensor, up: &Tensor) -> Result<Tensor> {
    // Delegate to candle-core's fused CUDA kernel implementation
    paramecia_core::deltanet_ops::fused_swiglu(gate, up)
}
...
fn parallel_indexed_matmul(
    ...
    let indices_u32 = indices_flat.to_dtype(paramecia_core::DType::U32)?;
...
mod tests {
    ...
    fn test_topk_moe_routing_cpu() -> Result<()> {
        ...
        let weights = paramecia_nn::ops::softmax_last_dim(&weights)?;
...
/// - Only works with CUDA or Metal backend
...
pub fn flash_attn_q8(
    q: &Tensor,
    k_storage: &paramecia_core::quantized::QStorage,
    v_storage: &paramecia_core::quantized::QStorage,
    q_l: &paramecia_core::Layout,
    k_l: &paramecia_core::Layout,
    v_l: &paramecia_core::Layout,
    scale: f32,
    batch: usize,
    num_heads: usize,
    num_kv_heads: usize,
