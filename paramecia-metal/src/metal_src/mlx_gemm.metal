...
struct GEMMKernel {
  ...
  static METAL_FUNC void run(
    ...
    // Explicitly zero threadgroup memory to avoid uninitialized data issues
    {
      const int tid_linear = simd_group_id * 32 + simd_lane_id;
      const int total_threads = WM * WN * 32;
      
      for (int i = tid_linear; i < tgp_mem_size_a; i += total_threads) {
        As[i] = T(0);
      }
      for (int i = tid_linear; i < tgp_mem_size_b; i += total_threads) {
        Bs[i] = T(0);
      }
    }
...
[[kernel, max_total_threads_per_threadgroup(WM* WN * 32)]] void gemm(
  ...
  // Explicitly zero threadgroup memory to avoid uninitialized data issues
  // This is especially important for edge cases with small M or N
  {
    const int tid = simd_group_id * 32 + simd_lane_id;
    const int total_threads = WM * WN * 32;
    
    // Zero As
    for (int i = tid; i < gemm_kernel::tgp_mem_size_a; i += total_threads) {
      As[i] = T(0);
    }
    // Zero Bs  
    for (int i = tid; i < gemm_kernel::tgp_mem_size_b; i += total_threads) {
      Bs[i] = T(0);
    }
  }
  ...
  // Only access addmm_params when use_out_source is true (buffer is bound)
  // Reading from unbound buffer causes undefined behavior
  const TransformAdd<AccumType, AccumType> epilogue_op_add(
      use_out_source ? addmm_params->alpha : 1.0f,
      use_out_source ? addmm_params->beta : 0.0f);
