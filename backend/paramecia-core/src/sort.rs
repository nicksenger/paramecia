...
#[cfg(feature = "cuda")]
fn contiguous_layout_context(layout: &crate::Layout) -> String {
    format!(
        "shape={:?}, stride={:?}, start_offset={}, rank={}, is_contiguous={}, is_fortran_contiguous={}, contiguous_offsets={:?}",
        layout.shape(),
        layout.stride(),
        layout.start_offset(),
        layout.shape().rank(),
    ...
    impl crate::cuda_backend::Map1Any for ArgSort {
        fn f<T: DeviceRepr + WithDType + ValidAsZeroBits, W: Fn(CudaSlice<T>) -> S>(
            ...
            let slice = match layout.contiguous_offsets() {
                None => {
                    return Err(crate::Error::RequiresContiguous { op: "argsort_cuda" }
                        .context(format!(
                        "ArgSort CUDA kernel requires contiguous input (asc={}, last_dim={}); {}",
                        self.asc,
                        self.last_dim,
                        super::contiguous_layout_context(layout),
                    ))
                        .bt())
                }
...
impl crate::CustomOp1 for ArgSort {
    ...
    fn vulkan_fwd(
        ...
        // Next power of 2 for ncols
        let mut ncols_pad = 1u32;
        while (ncols_pad as usize) < ncols {
            ncols_pad *= 2;
        }

        // sort.comp has local_size_x = 1024, shared_indices[1024]
        // Each thread handles one element in the bitonic sort, so ncols_pad <= 1024
        if ncols_pad > 1024 {
            return Err(crate::Error::Vulkan(
                "ArgSort Vulkan: ncols_pad > 1024 not supported, falling back".into(),
            ));
        }

        let shader_name = match dtype {
            crate::DType::F32 => "arg_sort_f32",
            crate::DType::U32 => "arg_sort_u32",
            crate::DType::BF16 => "arg_sort_bf16",
            crate::DType::F16 => "arg_sort_f16",
            crate::DType::U8 => "arg_sort_u8",
            crate::DType::I64 => "arg_sort_i64",
            dt => {
                return Err(crate::Error::Vulkan(
                    format!("ArgSort Vulkan: unsupported dtype {:?}", dt).into(),
                ));
            }
        };

        // Push constants: {base, nrows, ncols, ncols_pad, ascending} = 20 bytes
        #[repr(C)]
        struct SortPC {
            base: u32,
            nrows: u32,
        ...
        let pc = SortPC {
            ...
            ncols: ncols as u32,
            ncols_pad,
            ascending: if self.asc { 1 } else { 0 },
        };

        let pc_bytes = unsafe {
            std::slice::from_raw_parts(
                &pc as *const SortPC as *const u8,
                std::mem::size_of::<SortPC>(),
            )
        };

        // Output buffer: U32 indices (elem_count * 4 bytes)
        let output_buf = device.allocate_buffer((elem_count * 4) as u64)?;

        let pipeline = device
            .kernels()
            .load_pipeline(device.device(), shader_name, None, 20, 2, None, false)
            .map_err(|e| crate::Error::Msg(format!("Failed to load sort pipeline: {}", e)))?;

        let input_buf = storage.vk_buffer()?;
        device.record_compute(
            &pipeline,
            &[input_buf, output_buf.buffer],
            Some(pc_bytes),
            [nrows as u32, 1, 1],
        )?;

        let output_storage = crate::VulkanStorage::new(
            Some(std::sync::Arc::new(output_buf)),
            device.clone(),
            elem_count,
            crate::DType::U32,
        );

        Ok((output_storage, layout.shape().clone()))
    }
}
...
impl Tensor {
    ...
    pub fn arg_sort_last_dim(&self, asc: bool) -> Result<Tensor> {
        if !self.is_contiguous() {
            ...
            .context(format!(
                "arg_sort_last_dim called with non-contiguous tensor: shape={:?}, stride={:?}, start_offset={}, rank={}, dtype={:?}, device={:?}, asc={}",
                self.dims(),
                self.stride(),
    ...
    pub fn sort_last_dim(&self, asc: bool) -> Result<(Tensor, Tensor)> {
        if !self.is_contiguous() {
            ...
            .context(format!(
                "sort_last_dim called with non-contiguous tensor: shape={:?}, stride={:?}, start_offset={}, rank={}, dtype={:?}, device={:?}, asc={}",
                self.dims(),
                self.stride(),
                self.layout().start_offset(),
                self.rank(),
