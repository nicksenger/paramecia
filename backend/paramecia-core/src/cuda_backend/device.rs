...
use float8::F8E4M3;
use half::{bf16, f16};
pub use paramecia_cuda as kernels;
...
impl CudaDevice {
    ...
    /// Allocate page-locked (pinned) host memory for high-speed DMA transfers.
    ///
    /// Pinned memory enables:
    /// - Faster CPU<->GPU transfers via DMA
    /// - Async transfers that don't block the GPU
    /// - Overlap of computation with memory transfers
    ///
    /// # Safety
    ///
    /// The returned buffer must be used carefully:
    /// - Don't free it while a GPU transfer is in progress
    /// - The memory is allocated on the current CUDA context
    #[allow(clippy::missing_safety_doc)]
