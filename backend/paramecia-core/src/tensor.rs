...
impl Tensor {
    ...
    pub fn ones<S: Into<Shape>>(shape: S, dtype: DType, device: &Device) -> Result<Self> {
        Self::ones_impl(shape, dtype, device)
    }
    ...
    // Do not expose outside of the crate.
    ...
    pub fn zeros<S: Into<Shape>>(shape: S, dtype: DType, device: &Device) -> Result<Self> {
        Self::zeros_impl(shape, dtype, device)
    }
    ...
    pub unsafe fn empty<S: Into<Shape>>(shape: S, dtype: DType, device: &Device) -> Result<Self> {
        Self::empty_impl(shape, dtype, device)
    }
    ...
    pub fn rand<S: Into<Shape>, T: crate::FloatDType>(
        ...
        Self::rand_impl(lo, up, s, device)
    }

    pub fn rand_like(&self, lo: f64, up: f64) -> Result<Self> {
        Tensor::rand_f64_impl(lo, up, self.shape(), self.dtype(), self.device())
    }
    ...
    pub fn randn_like(&self, mean: f64, stdev: f64) -> Result<Self> {
        Tensor::randn_f64_impl(mean, stdev, self.shape(), self.dtype(), self.device())
    }
    ...
    pub fn randn<S: Into<Shape>, T: crate::FloatDType>(
        ...
        Self::randn_impl(mean, std, s, device)
    }
    ...
    pub fn from_storage<S: Into<Shape>>(storage: Storage, shape: S) -> Tensor {
    ...
    ///
    /// ```rust
    /// use paramecia_core::{Tensor, Device, Shape};
    ...
    fn reduce_impl<D: Dim>(&self, dim: D, keepdim: bool, op: ReduceOp) -> Result<Self> {
        ...
        dims[dim] = 1;
        let res = from_storage(storage, dims);
        if keepdim {
            Ok(res)
        } else {
    ...
    fn sum_impl<D: Dims>(&self, sum_dims: D, keepdim: bool) -> Result<Self> {
        ...
        let sum = from_storage(storage, dims);
        if keepdim {
            Ok(sum)
        } else {
    ...
    pub fn cmp<T: TensorOrScalar>(&self, rhs: T, op: CmpOp) -> Result<Self> {
        ...
        Ok(from_storage(storage, shape.dims()))
    }
    ...
    pub fn interpolate1d(&self, target_size: usize) -> Result<Self> {
        ...
        Ok(from_storage(storage, (n, c, target_size)))
    }
    ...
    pub fn interpolate2d(&self, target_h: usize, target_w: usize) -> Result<Self> {
        ...
        Ok(from_storage(storage, (n, c, target_h, target_w)))
    }
    ...
    pub fn upsample_bilinear2d_with_scale(
        ...
        Ok(from_storage(storage, (n, c, height_out, width_out)))
    }
    ...
    pub fn avg_pool2d_with_stride<T: crate::ToUsize2>(
        ...
        Ok(from_storage(storage, (n, c, h_out, w_out)))
    }
    ...
    pub fn gather<D: Dim>(&self, indexes: &Self, dim: D) -> Result<Self> {
        ...
        Ok(from_storage(storage, indexes.shape()))
    }
    ...
    /// Returns a new tensor detached from the current graph. With backprop removed, this is
    /// equivalent to clone.
        ...
        if self.device().same_device(device) {
            ...
            let storage = match (&*self.storage(), device) {
                ...
                (Storage::Vulkan(storage), Device::Vulkan(vk)) => {
                    let cpu_storage = storage.to_cpu_storage()?;
                    Storage::Vulkan(vk.storage_from_cpu_storage(&cpu_storage)?)
                }
    ...
    /// let tensor = tensor.to_dtype(paramecia_core::DType::F32)?;
    ...
    ///
    /// ```rust
    /// # use paramecia_core::{Tensor, DType, Device, D};
    /// let a = Tensor::zeros((2, 3), DType::F32, &Device::Cpu)?;
    ///
    /// let c = a.reshape((1, 6))?;
