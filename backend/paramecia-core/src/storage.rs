...
impl Storage {
    ...
    pub(crate) fn same_device(&self, rhs: &Self, op: &'static str) -> Result<()> {
        ...
        let same_device = if self.device().is_metal() || self.device().is_vulkan() {
            // On metal/vulkan, we require the device to be exactly the same rather than
    ...
    pub(crate) fn apply_op1(&self, l: &Layout, c: &dyn CustomOp1) -> Result<(Self, Shape)> {
        match self {
            ...
            Self::Vulkan(storage) => {
                match c.vulkan_fwd(storage, l) {
                    Ok((s, shape)) => Ok((Self::Vulkan(s), shape)),
                    Err(_e) => {
                        // CPU fallback for custom ops without vulkan_fwd impl
                        #[cfg(feature = "vulkan")]
                        {
                            use std::sync::atomic::{AtomicU64, Ordering};
                            static CUSTOMOP1_FALLBACK: AtomicU64 = AtomicU64::new(0);
                            let n = CUSTOMOP1_FALLBACK.fetch_add(1, Ordering::Relaxed);
                            if n < 100 {
                                warn!(
                                    "[VK FALLBACK] CustomOp1 '{}' shape={:?} err={}",
                                    c.name(),
                                    l.shape().dims(),
                                    _e
                                );
                        ...
                        let (cpu_result, shape) = c.cpu_fwd(&cpu, l)?;
                        let vk_result = storage.device().storage_from_cpu_storage(&cpu_result)?;
                        Ok((Self::Vulkan(vk_result), shape))
                    }
                }
            }
        }
    ...
    pub(crate) fn apply_op2(
        ...
        match (self, t2) {
            ...
            (Self::Vulkan(s1), Self::Vulkan(s2)) => {
                match c.vulkan_fwd(s1, l1, s2, l2) {
                    Ok((s, shape)) => Ok((Self::Vulkan(s), shape)),
                    Err(_) => {
                        #[cfg(feature = "vulkan")]
                        {
                            use std::sync::atomic::{AtomicU64, Ordering};
                            static CUSTOMOP2_FALLBACK: AtomicU64 = AtomicU64::new(0);
                            let n = CUSTOMOP2_FALLBACK.fetch_add(1, Ordering::Relaxed);
                            if n < 100 {
                                warn!(
                                    "[VK FALLBACK] CustomOp2 '{}' shape={:?}",
                                    c.name(),
                                    l1.shape().dims()
                                );
                            }
                        }
                        // Fallback to CPU for custom ops without Vulkan impl
                        use crate::backend::BackendStorage;
                        let cpu_s1 = s1.to_cpu_storage()?;
                        let cpu_s2 = s2.to_cpu_storage()?;
                        let (cpu_result, shape) = c.cpu_fwd(&cpu_s1, l1, &cpu_s2, l2)?;
                        let vk_result = s1.device().storage_from_cpu_storage(&cpu_result)?;
                        Ok((Self::Vulkan(vk_result), shape))
                    }
                }
            }
            ...
            (Self::Vulkan(s1), Self::Vulkan(s2), Self::Vulkan(s3)) => {
                match c.vulkan_fwd(s1, l1, s2, l2, s3, l3) {
                    ...
                    Err(_) => {
                        ...
                        {
                            use std::sync::atomic::{AtomicU64, Ordering};
                            static CUSTOMOP3_FALLBACK: AtomicU64 = AtomicU64::new(0);
                            let n = CUSTOMOP3_FALLBACK.fetch_add(1, Ordering::Relaxed);
                            if n < 100 {
                                warn!(
                                    "[VK FALLBACK] CustomOp3 '{}' shape={:?}",
                                    c.name(),
                                    l1.shape().dims()
                                );
                            }
                        ...
                        let (cpu_result, shape) =
                            c.cpu_fwd(&cpu_s1, l1, &cpu_s2, l2, &cpu_s3, l3)?;
                        let vk_result = s1.device().storage_from_cpu_storage(&cpu_result)?;
                        Ok((Self::Vulkan(vk_result), shape))
                    }
                }
        ...
        match self {
            ...
            Self::Vulkan(storage) => match c.vulkan_fwd(storage, l) {
                ...
                Err(_) => {
                    ...
                    c.cpu_fwd(&mut cpu, l)?;
                    let new_vk = storage.device().storage_from_cpu_storage(&cpu)?;
                    *storage = new_vk;
                    Ok(())
                }
            },
        ...
        match (self, t2) {
            ...
            (Self::Vulkan(s1), Self::Vulkan(s2)) => match c.vulkan_fwd(s1, l1, s2, l2) {
                ...
                Err(_) => {
                    ...
                    let cpu_s2 = s2.to_cpu_storage()?;
                    c.cpu_fwd(&mut cpu_s1, l1, &cpu_s2, l2)?;
                    let new_vk = s1.device().storage_from_cpu_storage(&cpu_s1)?;
                    *s1 = new_vk;
                    Ok(())
                }
            },
