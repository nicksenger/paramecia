...
impl LogitsProcessor {
    ...
    /// Get a reference to the current sampling strategy.
    ...
    fn sample_gumbel_softmax(&mut self, logits: &Tensor, temperature: f64) -> Result<u32> {
        let sampled =
            paramecia_nn::sampling::gumbel_softmax(logits, temperature, paramecia_core::D::Minus1)?;
    ...
    fn sample_topp(&mut self, prs: &mut [f32], top_p: f32, fallback: u32) -> Result<u32> {
    ...
    fn sample_topk(&mut self, prs: &mut [f32], top_k: usize, fallback: u32) -> Result<u32> {
        if top_k >= prs.len() {
            self.sample_multinomial(prs, fallback)
        } else {
            let mut argsort_indices = (0..prs.len()).collect::<Vec<_>>();
    ...
    pub fn sample_f(&mut self, logits: &Tensor, f: impl FnOnce(&mut [f32])) -> Result<u32> {
        ...
        let prs = |temperature: f64| -> Result<Vec<f32>> {
            ...
            let prs = paramecia_nn::ops::softmax_last_dim(&logits)?;
        ...
        let fallback = logits
            .argmax(paramecia_core::D::Minus1)?
            .to_scalar::<u32>()?;
        let next_token = match &self.sampling {
            Sampling::ArgMax => fallback,
            Sampling::GumbelSoftmax { temperature } => {
