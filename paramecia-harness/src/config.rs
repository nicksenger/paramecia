//! Configuration management for Paramecia.
...
pub struct ProviderConfig {
    ...
    /// Disable loading CONTEXT.txt for local backend conversations.
    #[serde(default)]
    pub local_disable_context: Option<bool>,
    /// Tool call format: "xml" (pure XML, default), "json_in_xml" (JSON-in-XML), or "preserve" (keep original format).
    /// "xml" format: `<tool_call><function=name><parameter=key>value</parameter></function></tool_call>`
    /// "json_in_xml" format: `<tool_call>{"name": "...", "arguments": {...}}</tool_call>`
    /// "preserve" format: keeps tool calls in their original format without reformatting
    #[serde(default)]
    pub tool_call_format: Option<String>,
    /// Include full conversation (system prompt, tool descriptions, CONTEXT.txt) in
    /// generation output files (.txt and .bin). Default: false (user/tool messages only).
    #[serde(default)]
...
/// Controller configuration for WASM controller runner.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ControllerConfig {
    /// Path to the inference model GGUF file.
    #[serde(default)]
    pub model_path: Option<PathBuf>,
    /// Path to a separate GGUF for training model.
    #[serde(default)]
    pub training_model_path: Option<PathBuf>,
    /// Path to a tokenizer.json file.
    #[serde(default)]
    pub tokenizer_path: Option<PathBuf>,
    /// Enable training interface alongside inference.
    #[serde(default)]
    pub training: bool,
...
fn default_training_lr() -> f64 {
    0.0001
}
fn default_training_epsilon() -> f64 {
    0.001
}
fn default_optimize_tensors() -> String {
    "all".to_string()
}
...
impl Default for ControllerConfig {
    fn default() -> Self {
        Self {
            ...
            training_lr: default_training_lr(),
            training_epsilon: default_training_epsilon(),
            checkpoint_dir: None,
            optimize_tensors: default_optimize_tensors(),
        }
    }
}
...
pub struct ParameciaConfig {
    ...
    /// Auto-compact threshold in tokens (when to trigger summarization).
    #[serde(default = "default_auto_compact_threshold")]
    pub auto_compact_threshold: u32,

    /// Maximum context length in tokens (default: 262144 / 256K).
    /// Set to 1048576 to enable YARN for 1M token context extension.
    /// This is used for the token display in the TUI and passed to the local backend.
    ...
    /// Controller configuration (WASM controller runner).
    #[serde(default)]
...
fn default_context_length() -> u32 {
    262_144 // 256K - native Qwen3-Next context length
}
...
fn default_providers() -> Vec<ProviderConfig> {
    vec![ProviderConfig {
        ...
        context_length: None,         // Defaults to 262144 (256K) in backend
        local_kv_cache_quant: None,   // Defaults to "q8_0" in backend
        local_layer_split: None,      // Single-GPU by default
        local_disable_context: None,  // Defaults to false in backend
        tool_call_format: None,       // Defaults to "xml" (QwenCoder format)
        full_generation_output: None, // Defaults to false (user/tool messages only)
        tuning_output: None,          // Defaults to false
        tuning_top_k: None,           // Defaults to 100
        tuning_tail_samples: None,    // Defaults to 20
    }]
}
...
impl Default for ParameciaConfig {
    fn default() -> Self {
        Self {
...
impl ParameciaConfig {
    ...
    pub fn load(agent: Option<&str>) -> ParameciaResult<Self> {
        ...
        if CONFIG_FILE.exists() {
            ...
            let file_config: ParameciaConfig = toml::from_str(&content)?;
    ...
    fn load_agent_config(name: &str) -> ParameciaResult<ParameciaConfig> {
        let path = AGENTS_DIR.join(format!("{name}.toml"));
        if !path.exists() {
            return Err(ParameciaError::Config(format!(
    ...
    fn validate(&self) -> ParameciaResult<()> {
    ...
    pub fn system_prompt(&self) -> ParameciaResult<String> {
        if self.system_prompt_id == "__none__" {
            return Ok(String::new());
        }
        load_prompt(&self.system_prompt_id).map_err(ParameciaError::Config)
    }
    ...
    pub fn get_active_model(&self) -> ParameciaResult<&ModelConfig> {
        self.models
            .iter()
            .find(|m| m.alias() == self.active_model)
            .ok_or_else(|| {
                ParameciaError::Config(format!(
    ...
    pub fn get_provider_for_model(&self, model: &ModelConfig) -> ParameciaResult<&ProviderConfig> {
        self.providers
            .iter()
    ...
    /// Get the tool call format for the active model's provider.
    /// Returns "xml" (default) or "json_in_xml".
    #[must_use]
    pub fn tool_call_format(&self) -> &str {
        if let Ok(model) = self.get_active_model()
            && let Ok(provider) = self.get_provider_for_model(model)
        {
            provider.tool_call_format.as_deref().unwrap_or("xml")
        } else {
            "xml"
        }
    }
    ...
    pub fn save(&self) -> ParameciaResult<()> {
        let content = toml::to_string_pretty(self)
            .map_err(|e| ParameciaError::Config(format!("Failed to serialize config: {e}")))?;
...
fn merge_configs(base: ParameciaConfig, overlay: ParameciaConfig) -> ParameciaConfig {
    ...
    ParameciaConfig {
        ...
        context_length: if overlay.context_length != default_context_length() {
            overlay.context_length
        } else {
            base.context_length
        },
        ...
        controller: merge_controller_configs(base.controller, overlay.controller),
    }
}

/// Merge controller configurations.
fn merge_controller_configs(base: ControllerConfig, overlay: ControllerConfig) -> ControllerConfig {
    ControllerConfig {
        model_path: overlay.model_path.or(base.model_path),
        training_model_path: overlay.training_model_path.or(base.training_model_path),
        tokenizer_path: overlay.tokenizer_path.or(base.tokenizer_path),
        training: overlay.training || base.training,
        minibatch_size: overlay.minibatch_size,
        training_lr: overlay.training_lr,
        training_epsilon: overlay.training_epsilon,
        checkpoint_dir: overlay.checkpoint_dir.or(base.checkpoint_dir),
        optimize_tensors: overlay.optimize_tensors,
    }
}
