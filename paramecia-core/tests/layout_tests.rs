use paramecia_core::{test_device, Device, IndexOp, Result, Tensor};
...
fn strided_blocks() -> Result<()> {
    use paramecia_core::Device::Cpu;
    let tensor = Tensor::arange(0u32, 24u32, &Cpu)?.reshape((2, 3, 4))?;
    match tensor.strided_blocks() {
        paramecia_core::StridedBlocks::SingleBlock { start_offset, len } => {
            assert_eq!(start_offset, 0);
            assert_eq!(len, 24);
        }
        paramecia_core::StridedBlocks::MultipleBlocks { .. } => {
    ...
    let tensor = tensor.i((.., 1))?;
    match tensor.strided_blocks() {
        paramecia_core::StridedBlocks::SingleBlock { .. } => {
            panic!("unexpected block structure")
        }
        paramecia_core::StridedBlocks::MultipleBlocks {
            block_len,
