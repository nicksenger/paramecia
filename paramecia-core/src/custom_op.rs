...
impl Tensor {
    ...
    pub fn apply_op1_arc(
        &self,
        c: std::sync::Arc<Box<dyn CustomOp1 + Send + Sync>>,
    ) -> Result<Self> {
    ...
    pub fn apply_op1<C: 'static + CustomOp1 + Send + Sync>(&self, c: C) -> Result<Self> {
        self.apply_op1_arc(std::sync::Arc::new(Box::new(c)))
    }
    ...
    pub fn apply_op2_arc(
        &self,
        rhs: &Self,
        c: std::sync::Arc<Box<dyn CustomOp2 + Send + Sync>>,
    ) -> Result<Self> {
    ...
    pub fn apply_op2<C: 'static + CustomOp2 + Send + Sync>(&self, r: &Self, c: C) -> Result<Self> {
        self.apply_op2_arc(r, std::sync::Arc::new(Box::new(c)))
    }
    ...
    pub fn apply_op3_arc(
        ...
        t2: &Self,
        t3: &Self,
        c: std::sync::Arc<Box<dyn CustomOp3 + Send + Sync>>,
    ) -> Result<Self> {
        let (storage, shape) = self.storage().apply_op3(
            self.layout(),
            &t2.storage(),
    ...
    pub fn apply_op3<C: 'static + CustomOp3 + Send + Sync>(
        ...
        self.apply_op3_arc(t2, t3, std::sync::Arc::new(Box::new(c)))
    }
}
