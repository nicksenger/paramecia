...
pub fn nll(inp: &Tensor, target: &Tensor) -> Result<Tensor> {
    let b_sz = match target.dims() {
        &[b_sz] => b_sz,
        dims => {
            paramecia_core::bail!("the target tensor should have a single dimension ({dims:?})")
        }
    };
    match inp.dims() {
        &[inp_b_sz, _] => {
        ...
        dims => paramecia_core::bail!("the target tensor should have two dimensions ({dims:?})"),
    }
...
pub fn cross_entropy(inp: &Tensor, target: &Tensor) -> Result<Tensor> {
    if inp.rank() != 2 {
        paramecia_core::bail!("cross_entropy expects an input tensor of rank 2")
    }
