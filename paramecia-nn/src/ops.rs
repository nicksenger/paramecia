...
use paramecia_core::{CpuStorage, DType, Layout, Module, Result, Shape, Tensor, D};
use rayon::prelude::*;
...
///
/// ```rust
/// use paramecia_core::{Tensor, Device, test_utils::to_vec2_round};
/// let a = Tensor::new(&[[0f32, 1., 0., 1.], [-2., 2., 3., -3.]], &Device::Cpu)?;
/// let a = paramecia_nn::ops::softmax(&a, 1)?;
/// assert_eq!(
///     to_vec2_round(&a, 4)?,
///     &[
...
/// ```
pub fn softmax<D: paramecia_core::shape::Dim>(xs: &Tensor, dim: D) -> Result<Tensor> {
...
pub fn log_softmax<D: paramecia_core::shape::Dim>(xs: &Tensor, d: D) -> Result<Tensor> {
...
impl paramecia_core::CustomOp1 for Sigmoid {
    ...
    fn cpu_fwd(&self, storage: &CpuStorage, layout: &Layout) -> Result<(CpuStorage, Shape)> {
        ...
        // FIXME: using `paramecia_core::map_dtype` causes compilation errors.
        let storage = match storage {
            CpuStorage::BF16(slice) => {
                CpuStorage::BF16(paramecia_core::cpu_backend::unary_map(slice, layout, fwd))
            }
            CpuStorage::F16(slice) => {
                CpuStorage::F16(paramecia_core::cpu_backend::unary_map(slice, layout, fwd))
            }
            CpuStorage::F32(slice) => {
                CpuStorage::F32(paramecia_core::cpu_backend::unary_map(slice, layout, fwd))
            }
            CpuStorage::F64(slice) => {
                CpuStorage::F64(paramecia_core::cpu_backend::unary_map(slice, layout, fwd))
            }
            _ => Err(paramecia_core::Error::UnsupportedDTypeForOp(
                storage.dtype(),
                self.name(),
            ))?,
        };
    ...
    fn cuda_fwd(
        &self,
        storage: &paramecia_core::CudaStorage,
        layout: &Layout,
    ) -> Result<(paramecia_core::CudaStorage, Shape)> {
        use paramecia_core::backend::BackendStorage;
        use paramecia_core::cuda_backend::cudarc::driver::{
            CudaSlice, DeviceRepr, LaunchConfig, PushKernelArg, ValidAsZeroBits,
        };
        use paramecia_core::cuda_backend::SlicePtrOrNull;
        use paramecia_core::cuda_backend::{kernel_name, kernels, Map1, WrapErr};
        use paramecia_core::{CudaDevice, WithDType};
        ...
        impl Map1 for S {
            fn f<T: DeviceRepr + WithDType + ValidAsZeroBits>(
                ...
                paramecia_core::builder_arg!(builder, el_count, dims.len());
        ...
        let dst = paramecia_core::CudaStorage {
            slice,
            device: dev.clone(),
        };
    ...
    fn metal_fwd(
        &self,
        storage: &paramecia_core::MetalStorage,
        layout: &Layout,
    ) -> Result<(paramecia_core::MetalStorage, Shape)> {
        use paramecia_core::backend::BackendStorage;
        use paramecia_core::MetalError;
        ...
        let src = paramecia_metal::BufferOffset {
            buffer: storage.buffer(),
            offset_in_bytes: layout.start_offset() * storage.dtype().size_in_bytes(),
        };

        if layout.is_contiguous() {
            use paramecia_metal::unary::contiguous;
            let kernel_name = match dtype {
                ...
                dtype => {
                    paramecia_core::bail!(
                        "Metal contiguous unary sigmoid {dtype:?} not implemented"
                    )
                }
            };
            paramecia_metal::call_unary_contiguous(
                device.metal_device(),
                &encoder,
                device.kernels(),
                kernel_name,
            ...
            use paramecia_metal::unary::strided;
            let kernel_name = match dtype {
                DType::F16 => strided::sigmoid::HALF,
                DType::F32 => strided::sigmoid::FLOAT,
                DType::BF16 => strided::sigmoid::BFLOAT,
                dtype => {
                    paramecia_core::bail!("Metal strided unary sigmoid {dtype:?} not implemented")
                }
            };
            let dst = paramecia_metal::BufferOffset::zero_offset(&buffer);
            paramecia_metal::call_unary_strided(
                device.metal_device(),
                &encoder,
                device.kernels(),
                kernel_name,
        ...
        let new_storage =
            paramecia_core::MetalStorage::new(buffer, device.clone(), el_count, dtype);
    ...
    fn vulkan_fwd(
        &self,
        storage: &paramecia_core::VulkanStorage,
        layout: &Layout,
    ) -> Result<(paramecia_core::VulkanStorage, Shape)> {
        use paramecia_core::backend::{BackendDevice, BackendStorage};
        ...
        let dtype_suffix = match dtype {
            ...
            DType::F16 => "f16",
            DType::BF16 => "bf16",
            _ => paramecia_core::bail!("sigmoid not implemented for {:?} on Vulkan", dtype),
        };

        let num_elements = layout.shape().elem_count();

        // Use the same push constant layout as Vulkan unary_impl
        ...
        let rank = dims.len();
        let mut shape = [1u32; MAX_RANK];
        let mut stride = [1u32; MAX_RANK];
        for i in 0..rank.min(MAX_RANK) {
            shape[i] = dims[i] as u32;
        }
        for i in 0..strides.len().min(MAX_RANK) {
            stride[i] = strides[i] as u32;
        }
        ...
        let key = format!("sigmoid_{}", dtype_suffix);
        let pipeline = storage
            ...
            .map_err(|e| {
                paramecia_core::Error::Msg(format!("Failed to load {} pipeline: {}", key, e))
            })?;

        let dst = unsafe { storage.device().alloc_uninit(layout.shape(), dtype)? };

        let groups = ((num_elements as u32) + 255) / 256;
        storage.device().record_compute(
            &pipeline,
            &[storage.vk_buffer()?, dst.vk_buffer()?],
            Some(pc_bytes),
            [groups, 1, 1],
        )?;
...
pub fn dropout(xs: &Tensor, drop_p: f32) -> Result<Tensor> {
    ...
    if !(0. ..1.).contains(&drop_p) {
        paramecia_core::bail!("dropout probability has to be in [0, 1), got {drop_p}")
    }
...
impl paramecia_core::ModuleT for Dropout {
...
impl paramecia_core::CustomOp1 for SoftmaxLastDim {
    fn name(&self) -> &'static str {
        "softmax-last-dim"
    }

    fn cpu_fwd(&self, storage: &CpuStorage, layout: &Layout) -> Result<(CpuStorage, Shape)> {
        fn softmax<T: paramecia_core::WithDType + num_traits::Float>(
            src: &[T],
            layout: &Layout,
        ) -> Result<(CpuStorage, Shape)> {
            let src = match layout.contiguous_offsets() {
                None => paramecia_core::bail!(
                    "input has to be contiguous (softmax cpu), shape={:?}, stride={:?}, start_offset={}",
                    layout.shape(),
                    layout.stride(),
                    layout.start_offset()
                ),
            ...
            let storage = paramecia_core::WithDType::to_cpu_storage_owned(dst);
        ...
        match storage {
            ...
            _ => paramecia_core::bail!("unsupported dtype for softmax {:?}", storage),
        }
    }
    ...
    fn cuda_fwd(
        ...
        impl Map1 for S {
            fn f<T: DeviceRepr + WithDType>(
                ...
                let src = match layout.contiguous_offsets() {
                    None => paramecia_core::bail!(
                        "input has to be contiguous (softmax cuda), shape={:?}, stride={:?}, start_offset={}",
                        layout.shape(),
                        layout.stride(),
                        layout.start_offset()
                    ),
                ...
                builder.arg(&src);
                builder.arg(&dst);
                paramecia_core::builder_arg!(builder, n_cols as i32);
                // SAFETY: ffi.
                unsafe { builder.launch(cfg) }.w()?;
                Ok(dst)
            }
        ...
        let dev = storage.device();
        let slice = S.map(&storage.slice, dev, layout)?;
        let dst = paramecia_core::cuda_backend::CudaStorage {
            slice,
            device: dev.clone(),
        };
        Ok((dst, layout.shape().clone()))
    ...
    fn metal_fwd(
        ...
        let name = match storage.dtype() {
            ...
            dtype => paramecia_core::bail!("softmax-last-dim is not implemented for {dtype:?}"),
        };

        let n = layout.stride().len();
        if !(layout.is_contiguous() && layout.stride()[n - 1] == 1) {
            paramecia_core::bail!("Non contiguous softmax-last-dim is not implemented");
        }

        let last_dim = layout.dims()[layout.shape().rank() - 1];
        let elem_count = layout.shape().elem_count();
        let output = device.new_buffer(elem_count, storage.dtype(), "softmax")?;
        paramecia_metal::call_last_softmax(
            device.metal_device(),
            &encoder,
            kernels,
            name,
        ...
        .map_err(paramecia_core::Error::wrap)?;
        let newstorage =
            paramecia_core::MetalStorage::new(output, device.clone(), elem_count, storage.dtype());
    ...
    fn vulkan_fwd(
        ...
        if !layout.is_contiguous() {
            paramecia_core::bail!("Non contiguous softmax-last-dim is not implemented for Vulkan");
        }
        ...
        let dtype_suffix = match dtype {
            ...
            DType::F16 => "f16",
            DType::BF16 => "bf16",
            _ => paramecia_core::bail!("softmax not implemented for {:?} on Vulkan", dtype),
        };

        let dims = layout.shape().dims();
        let rank = dims.len();
        let axis = rank - 1;
        let last_dim = dims[axis];
        let elem_count = layout.shape().elem_count();
        let num_slices = elem_count / last_dim;

        // Build push constants matching softmax.comp
        #[repr(C)]
        #[derive(Clone, Copy)]
        struct SoftmaxPC {
            base_offset: u32,
        ...
        let mut stride = [1u32; 4];
        for i in 0..rank.min(4) {
            shape[i] = dims[i] as u32;
            stride[i] = layout.stride()[i] as u32;
        }
        ...
        let pc_bytes = unsafe {
            std::slice::from_raw_parts(
                ...
                std::mem::size_of::<SoftmaxPC>(),
            )
        };

        let key = format!("softmax_{}", dtype_suffix);
        let pipeline = storage
            .device()
            .kernels()
            .load_pipeline(
                ...
                &key,
                None,
                std::mem::size_of::<SoftmaxPC>() as u32,
                2,
                None,
                false,
            )
        ...
        // Each thread handles one full row - dispatch ceil(num_slices/256) workgroups
        storage.device().record_compute(
            &pipeline,
            &[storage.vk_buffer()?, dst.vk_buffer()?],
            Some(pc_bytes),
            [((num_slices as u32) + 255) / 256, 1, 1],
        )?;
...
impl paramecia_core::CustomOp2 for RmsNorm {
    fn name(&self) -> &'static str {
        "rms-norm"
    }
    ...
    fn cpu_fwd(
        ...
        >(
            ...
            let src = match layout.contiguous_offsets() {
                None => paramecia_core::bail!(
                    "input has to be contiguous (rmsnorm cpu src), shape={:?}, stride={:?}, start_offset={}",
                    layout.shape(),
                    layout.stride(),
                    layout.start_offset()
                ),
            ...
            let alpha = match alpha_layout.contiguous_offsets() {
                None => paramecia_core::bail!("alpha has to be contiguous"),
        ...
        match (s1, s2) {
            ...
            _ => paramecia_core::bail!("unsupported dtype for rmsnorm {:?}", s1.dtype()),
        }
    }
    ...
    fn cuda_fwd(
        &self,
        s1: &paramecia_core::CudaStorage,
        l1: &Layout,
        s2: &paramecia_core::CudaStorage,
        l2: &Layout,
    ) -> Result<(paramecia_core::CudaStorage, Shape)> {
        use paramecia_core::cuda_backend::cudarc::driver::{
            CudaSlice, DeviceRepr, LaunchConfig, PushKernelArg,
        };
        use paramecia_core::cuda_backend::{kernel_name, kernels, Map2, WrapErr};
        use paramecia_core::{CudaDevice, WithDType};
        ...
        impl Map2 for S {
            fn f<T: DeviceRepr + WithDType>(
                ...
                let src = match layout.contiguous_offsets() {
                    None => paramecia_core::bail!(
                        "input has to be contiguous (rmsnorm cuda src), shape={:?}, stride={:?}, start_offset={}",
                        layout.shape(),
                        layout.stride(),
                        layout.start_offset()
                    ),
                ...
                builder.arg(&dst);
                builder.arg(&alpha);
                paramecia_core::builder_arg!(builder, n_cols as i32, block_size as i32, self.eps);
                // SAFETY: ffi.
                unsafe { builder.launch(cfg) }.w()?;
                Ok(dst)
            }
    ...
    fn metal_fwd(
        &self,
        s1: &paramecia_core::MetalStorage,
        l1: &Layout,
        s2: &paramecia_core::MetalStorage,
        l2: &Layout,
    ) -> Result<(paramecia_core::MetalStorage, Shape)> {
        use paramecia_core::backend::BackendStorage;
        ...
        let name = match (s1.dtype(), s2.dtype()) {
            ...
            (dt1, dt2) => paramecia_core::bail!("rmsnorm is not implemented for {dt1:?} {dt2:?}"),
        };

        if !(l1.is_contiguous() && l2.is_contiguous()) {
            paramecia_core::bail!("Non contiguous rmsnorm is not implemented");
        }

        let last_dim = l1.dims()[l1.shape().rank() - 1];
        let elem_count = l1.shape().elem_count();
        let output = device.new_buffer(elem_count, s1.dtype(), "rmsnorm")?;
        paramecia_metal::call_rms_norm(
            device.metal_device(),
            &encoder,
            kernels,
            name,
        ...
        .map_err(paramecia_core::Error::wrap)?;
        let newstorage =
            paramecia_core::MetalStorage::new(output, device.clone(), elem_count, s1.dtype());
    ...
    fn vulkan_fwd(
        &self,
        s1: &paramecia_core::VulkanStorage,
        l1: &Layout,
        s2: &paramecia_core::VulkanStorage,
        l2: &Layout,
    ) -> Result<(paramecia_core::VulkanStorage, Shape)> {
        use paramecia_core::backend::{BackendDevice, BackendStorage};

        if !(l1.is_contiguous() && l2.is_contiguous()) {
            paramecia_core::bail!("Non contiguous rmsnorm is not implemented for Vulkan");
        }
        ...
        let dtype_suffix = match dtype {
            ...
            DType::F16 => "f16",
            DType::BF16 => "bf16",
            _ => paramecia_core::bail!("rmsnorm not implemented for {:?} on Vulkan", dtype),
        };

        let dims = l1.shape().dims();
        let rank = dims.len();
        let axis = rank - 1;
        let norm_dim_size = dims[axis];
        let elem_count = l1.shape().elem_count();
        let num_slices = elem_count / norm_dim_size;

        // Build push constants matching rmsnorm.comp
        #[repr(C)]
        #[derive(Clone, Copy)]
        struct RmsNormPC {
            in_base: u32,
        ...
        let mut in_shape = [1u32; 4];
        let mut in_stride = [1u32; 4];
        for i in 0..rank.min(4) {
            in_shape[i] = dims[i] as u32;
            in_stride[i] = l1.stride()[i] as u32;
        }

        // Output is contiguous
        let out_shape = in_shape;
        let mut out_stride = [1u32; 4];
        {
            let mut s = 1u32;
            for i in (0..rank.min(4)).rev() {
                out_stride[i] = s;
                s *= out_shape[i];
            }
        }
        ...
        let pc = RmsNormPC {
            in_base: l1.start_offset() as u32,
            rank: rank as u32,
            axis: axis as u32,
            eps: self.eps,
            in_shape,
            ...
            out_shape,
            out_stride,
            norm_dim_size: norm_dim_size as u32,
            _pad: [0; 3],
        };
        ...
        let pc_bytes = unsafe {
            std::slice::from_raw_parts(
                &pc as *const _ as *const u8,
                std::mem::size_of::<RmsNormPC>(),
            )
        };

        let key = format!("rmsnorm_{}", dtype_suffix);
        let pipeline = s1
            .device()
            .kernels()
            .load_pipeline(
                ...
                &key,
                None,
                std::mem::size_of::<RmsNormPC>() as u32,
                3,
                None,
                false,
            )
        ...
        let dst = unsafe { s1.device().alloc_uninit(l1.shape(), dtype)? };

        s1.device().record_compute(
            &pipeline,
            &[s1.vk_buffer()?, s2.vk_buffer()?, dst.vk_buffer()?],
            Some(pc_bytes),
...
impl paramecia_core::CustomOp3 for LayerNorm {
    fn name(&self) -> &'static str {
        "layer-norm"
    }
    ...
    fn cpu_fwd(
        ...
        >(
            ...
            let src = match layout.contiguous_offsets() {
                None => paramecia_core::bail!(
                    "input has to be contiguous (layernorm cpu src), shape={:?}, stride={:?}, start_offset={}",
                    layout.shape(),
                    layout.stride(),
                    layout.start_offset()
                ),
            ...
            let beta = match beta_layout.contiguous_offsets() {
                None => paramecia_core::bail!("beta has to be contiguous"),
    ...
    fn cuda_fwd(
        ...
        s2: &paramecia_core::CudaStorage,
        l2: &Layout,
        s3: &paramecia_core::CudaStorage,
        l3: &Layout,
    ) -> Result<(paramecia_core::CudaStorage, Shape)> {
        use paramecia_core::cuda_backend::cudarc::driver::{
            CudaSlice, DeviceRepr, LaunchConfig, PushKernelArg,
        };
        use paramecia_core::cuda_backend::{kernel_name, kernels, Map3, WrapErr};
        use paramecia_core::{CudaDevice, WithDType};
        ...
        impl Map3 for S {
            fn f<T: DeviceRepr + WithDType>(
                ...
                let src = match layout.contiguous_offsets() {
                    None => paramecia_core::bail!(
                        "input has to be contiguous (layernorm cuda src), shape={:?}, stride={:?}, start_offset={}",
                        layout.shape(),
                        layout.stride(),
                        layout.start_offset()
                    ),
    ...
    fn metal_fwd(
        ...
        s2: &paramecia_core::MetalStorage,
        l2: &Layout,
        s3: &paramecia_core::MetalStorage,
        l3: &Layout,
    ) -> Result<(paramecia_core::MetalStorage, Shape)> {
        use paramecia_core::backend::BackendStorage;
        let device = s1.device();
        ...
        let name = match (s1.dtype(), s2.dtype(), s3.dtype()) {
            ...
            (dt1, dt2, dt3) => {
                paramecia_core::bail!("layernorm is not implemented for {dt1:?} {dt2:?} {dt3:?}")
            }
        };

        if !(l1.is_contiguous() && l2.is_contiguous() && l3.is_contiguous()) {
            paramecia_core::bail!("Non contiguous layernorm is not implemented");
        }

        let last_dim = l1.dims()[l1.shape().rank() - 1];
        let elem_count = l1.shape().elem_count();
        let output = device.new_buffer(elem_count, s1.dtype(), "layernorm")?;
        paramecia_metal::call_layer_norm(
            device.metal_device(),
            &encoder,
            kernels,
            name,
...
pub fn replication_pad2d(xs: &Tensor, pad: usize) -> Result<Tensor> {
    match pad {
        ...
        n => paramecia_core::bail!("replication-pad with a size of {n} is not supported"),
    }
}
...
impl paramecia_core::CustomOp3 for Sdpa {
    fn name(&self) -> &'static str {
        "metal-sdpa"
    }
    ...
    fn cpu_fwd(
        ...
        paramecia_core::bail!("SDPA has no cpu impl")
    }

    #[cfg(feature = "metal")]
    fn metal_fwd(
        &self,
        q: &paramecia_core::MetalStorage,
        q_l: &Layout,
        k: &paramecia_core::MetalStorage,
        k_l: &Layout,
        v: &paramecia_core::MetalStorage,
        v_l: &Layout,
    ) -> Result<(paramecia_core::MetalStorage, Shape)> {
        use paramecia_core::backend::BackendStorage;
        use paramecia_metal::SdpaDType;
        ...
        if q_l.dim(D::Minus1)? != k_l.dim(D::Minus1)? {
            paramecia_core::bail!("`q` and `k` last dims must match");
        }

        // k,v must have matching n kv heads
        if v_l.dim(D::Minus(3))? != k_l.dim(D::Minus(3))? {
            paramecia_core::bail!("`k` and `v` head dims must match");
        }

        // n_heads % n_kv_heads == 0; n_heads >= 1, n_kv_heads >= 1.
        if q_l.dim(D::Minus(3))? % k_l.dim(D::Minus(3))? != 0 {
            paramecia_core::bail!("query `n_heads` must be a multiple of `n_kv_heads`");
        }
        ...
        for t in [k.dtype(), v.dtype()] {
            if q.dtype() != t {
                paramecia_core::bail!("all q, k, v dtypes must match.");
            }
        }
        ...
        let itype = match q.dtype() {
            ...
            other => paramecia_core::bail!("unsupported sdpa type {other:?}"),
        };
        ...
        if supports_sdpa_vector {
            ...
            if k_seq >= TWO_PASS_K_THRESHOLD {
                let mut intermediate_shape = [
                    ...
                    &[paramecia_metal::SDPA_2PASS_BLOCKS],
                ...
                paramecia_metal::call_sdpa_vector_2pass(
                    q.device().device(),
                    &encoder,
                    q.device().kernels(),
                    q_l.start_offset(),
                ...
                encoder.set_label("vector_attention");
                paramecia_metal::call_sdpa_vector(
                    q.device().device(),
                    &encoder,
                    q.device().kernels(),
                    q_l.start_offset(),
            ...
            if self.softcapping != 1. {
                paramecia_core::bail!("SDPA full requires softcapping to be disabled (1.0)");
            }
            ...
            let (mask_type, mask_buffer, mask_strides) = if let Some(mask) = &self.mask {
                ...
                let mask_buffer = match &**mask_s {
                    paramecia_core::Storage::Metal(m) => m.buffer(),
                    _ => paramecia_core::bail!("Expected metal device for mask"),
                };
                ...
                if mask_type != itype {
                    paramecia_core::bail!("Mask type {mask_type:?} must match q type {itype:?}");
                }
            ...
            paramecia_metal::call_sdpa_full(
                q.device().device(),
                &encoder,
                q.device().kernels(),
                q_l.start_offset(),
            ...
            paramecia_core::bail!("must be vector or full sdpa kernel");
        }

        let newstorage =
            paramecia_core::MetalStorage::new(output, device.clone(), elem_count, q.dtype());
