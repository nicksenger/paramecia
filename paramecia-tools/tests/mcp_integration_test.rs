...
impl paramecia_tools::mcp::transport::Transport for MockTransport {
    async fn send(&self, _request: &str) -> paramecia_tools::mcp::McpResult<String> {
    ...
    async fn close(&self) -> paramecia_tools::mcp::McpResult<()> {
        Ok(())
    }
}
