...
pub struct SearchReplaceResult {
    ...
    pub content: String,
    /// Unified diff for the applied change, if any change was made.
...
impl Tool for SearchReplace {
    ...
    async fn execute(&mut self, args: serde_json::Value) -> ToolResult<serde_json::Value> {
        ...
        let unified_diff = if result.content == original_content {
            None
        } else {
            Some(diffy::create_patch(&original_content, &result.content).to_string())
        };
