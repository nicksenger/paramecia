...
pub fn render_tool_result(msg: &ToolResultMessage, width: u16) -> Vec<Line<'static>> {
    if let Some(lines) = render_search_replace_diff(msg, width) {
        return lines;
    }
    ...
    let mut content_lines: Vec<Line<'static>> = display_text
        .lines()
        .map(|line| Line::from(vec![Span::styled(line.to_string(), text_style)]))
        .collect();

    if content_lines.is_empty() {
        content_lines.push(Line::from(vec![Span::styled(display_text, text_style)]));
    }

    add_tool_result_border(content_lines)
}
...
fn add_tool_result_border(content_lines: Vec<Line<'static>>) -> Vec<Line<'static>> {
    let border_style = Style::default().fg(colors::MUTED);
    ...
    for (i, line) in content_lines.into_iter().enumerate() {
        ...
        spans.push(Span::styled(format!("  {border} "), border_style));
...
fn diff_line_number_width(lines: &[DiffRenderLine]) -> usize {
    lines
        .iter()
        .filter_map(|line| line.line_number)
        .max()
        .map_or(1, |max_line| max_line.to_string().len())
}

fn parse_hunk_starts(line: &str) -> Option<(usize, usize)> {
    let mut parts = line.split_whitespace();
    let old_part = parts.find(|part| part.starts_with('-'))?;
    let new_part = parts.find(|part| part.starts_with('+'))?;
...
fn parse_unified_diff(unified_diff: &str) -> Vec<DiffRenderLine> {
    let mut lines = Vec::new();
    let mut old_line = 0usize;
    let mut new_line = 0usize;

    for raw_line in unified_diff.lines() {
        ...
        if let Some(text) = raw_line.strip_prefix('+') {
            lines.push(DiffRenderLine {
                kind: DiffRenderKind::Insert,
                line_number: if new_line == 0 { None } else { Some(new_line) },
                text: text.to_string(),
            });
        ...
        if let Some(text) = raw_line.strip_prefix('-') {
            lines.push(DiffRenderLine {
                kind: DiffRenderKind::Delete,
                line_number: if old_line == 0 { None } else { Some(old_line) },
                text: text.to_string(),
            });
...
fn diff_change_counts(lines: &[DiffRenderLine]) -> (usize, usize) {
    let mut inserts = 0usize;
    let mut deletes = 0usize;

    for line in lines {
        match line.kind {
            DiffRenderKind::Insert => {
                inserts = inserts.saturating_add(1);
            }
            DiffRenderKind::Delete => {
                deletes = deletes.saturating_add(1);
            }
            DiffRenderKind::Context | DiffRenderKind::Hunk => {}
        }
    }
...
fn push_wrapped_diff_line(
    ...
    let prefix_cols = gutter_width + 2;
    let mut first = true;

    loop {
        let available_cols = width.saturating_sub(prefix_cols).max(1);
        let (chunk, rest) = split_text_at_width(remaining, available_cols);
        ...
        if first {
            ...
            out.push(Line::from(vec![
                ...
                Span::styled(format!("{sign}{chunk_text}"), line_style),
            ]));
            first = false;
        } else {
            out.push(Line::from(vec![
                Span::styled(format!("{:gutter_width$}  ", ""), gutter_style),
                Span::styled(chunk_text, line_style),
            ]));
        }
...
fn render_search_replace_diff(msg: &ToolResultMessage, width: u16) -> Option<Vec<Line<'static>>> {
    if msg.collapsed || msg.tool_name != "search_replace" {
        return None;
    }

    let result_object = msg.result.as_ref()?.as_object()?;
    let unified_diff = result_object.get("unified_diff")?.as_str()?;
    if unified_diff.trim().is_empty() {
        return None;
    }

    let parsed_lines = parse_unified_diff(unified_diff);
    if parsed_lines.is_empty() {
        return None;
    }

    let (insert_count, delete_count) = diff_change_counts(&parsed_lines);
    let mut content_lines: Vec<Line<'static>> = Vec::new();
    content_lines.push(Line::from(vec![
        Span::raw("search_replace completed "),
    ...
    let line_number_width = diff_line_number_width(&parsed_lines);
    let available_width = usize::from(width.saturating_sub(4)).max(16);

    for parsed in parsed_lines {
        match parsed.kind {
            DiffRenderKind::Insert => push_wrapped_diff_line(
                &mut content_lines,
                parsed.line_number,
                '+',
                &parsed.text,
            ...
            DiffRenderKind::Delete => push_wrapped_diff_line(
                &mut content_lines,
                parsed.line_number,
                '-',
                &parsed.text,
            ...
            DiffRenderKind::Context => push_wrapped_diff_line(
                &mut content_lines,
                parsed.line_number,
                ' ',
                &parsed.text,
            ...
            DiffRenderKind::Hunk => content_lines.push(Line::from(vec![Span::styled(
                parsed.text,
                Style::default()
                    .fg(colors::MUTED)
    ...
    Some(add_tool_result_border(content_lines))
}
