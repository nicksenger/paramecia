...
impl InputState {
    ...
    pub fn delete_word(&mut self) {
        ...
        let word_start = self.beginning_of_previous_word();
        if word_start < cursor {
            self.content.replace_range(word_start..cursor, "");
    ...
    /// Move to the beginning of the previous word (byte offset).
    fn beginning_of_previous_word(&self) -> usize {
        let cursor = self.safe_cursor();
        let prefix = &self.content[..cursor];
        ...
        let separator_word = is_word_separator(ch);
        let mut start = first_non_ws_idx;

        for (idx, ch) in prefix[..first_non_ws_idx].char_indices().rev() {
            if ch.is_whitespace() || is_word_separator(ch) != separator_word {
    ...
    /// Move to the end of the next word (byte offset).
    fn end_of_next_word(&self) -> usize {
        let cursor = self.safe_cursor();
        let Some(first_non_ws) = self.content[cursor..].find(|c: char| !c.is_whitespace()) else {
            return self.content.len();
        };

        let word_start = cursor + first_non_ws;
        let mut iter = self.content[word_start..].char_indices();
        let Some((_, first_ch)) = iter.next() else {
            return word_start;
        };

        let separator_word = is_word_separator(first_ch);
