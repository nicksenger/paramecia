...
use crossterm::{
    ...
    event::{self, KeyCode, KeyModifiers, MouseEvent, MouseEventKind},
    execute,
...
pub struct App {
    ...
    /// Monotonic progress counter for loading wave animation.
    pub loading_wave_progress: usize,
    /// Chat scroll offset.
    ...
    /// Active tool calls mapped by tool call ID to their message index.
    pub active_tool_call_indices: HashMap<String, usize>,
...
impl App {
    ...
    pub fn new(config: ParameciaConfig, mode: AgentMode) -> Self {
        ...
        Self {
            ...
            color_index: 0,
            loading_wave_progress: 0,
            scroll_offset: 0,
            max_scroll: 0,
            auto_scroll: true,
            active_tool_call_indices: HashMap::new(),
            approval: None,
            token_context: (0, 0),
    ...
    pub fn stop_loading(&mut self) {
        ...
        self.loading_wave_progress = 0;
        self.stop_all_tool_call_spinners();
    }

    /// Stop a specific tool call spinner by message index.
    fn stop_tool_call_spinner_by_index(&mut self, idx: usize) {
        if let Some(Message::ToolCall(msg)) = self.messages.get_mut(idx) {
            msg.stop();
        }
    }

    /// Stop all active tool call spinners.
    fn stop_all_tool_call_spinners(&mut self) {
        let active_indices: Vec<usize> = self.active_tool_call_indices.values().copied().collect();
        for idx in active_indices {
            self.stop_tool_call_spinner_by_index(idx);
        }
        self.active_tool_call_indices.clear();
    }

    /// Stop the most recent spinning tool call, optionally filtered by tool name.
    fn stop_most_recent_spinning_tool_call(&mut self, tool_name: Option<&str>) {
        if let Some((idx, _)) = self
            ...
            .find(|(_, msg)| match msg {
                Message::ToolCall(tool_call) => {
                    tool_call.spinning && tool_name.is_none_or(|name| name == tool_call.tool_name)
                }
                _ => false,
            })
        {
            ...
            self.active_tool_call_indices
                .retain(|_, mapped_idx| *mapped_idx != idx);
        }
    }
    ...
    pub fn add_tool_call(
        ...
        let msg = ToolCallMessage::new(tool_name, tool_call_id.clone(), args);
        self.messages.push(Message::ToolCall(msg));
        self.active_tool_call_indices
            .insert(tool_call_id, self.messages.len() - 1);
        if self.auto_scroll {
            self.anchor_scroll();
        }
    }
    ...
    pub fn add_tool_result(&mut self, event: &ToolResultEvent) {
        if let Some(idx) = self.active_tool_call_indices.remove(&event.tool_call_id) {
            self.stop_tool_call_spinner_by_index(idx);
        } else {
            self.stop_most_recent_spinning_tool_call(Some(&event.tool_name));
        }

        let mut msg = if event.error.is_some() {
...
{}",
    ...
    pub fn tick(&mut self) {
        let now = Instant::now();
        self.context_progress.tick(self.loading);
        ...
        if now.duration_since(self.last_render) >= Duration::from_millis(100) {
            ...
            if self.loading {
                self.loading_wave_progress = self.loading_wave_progress.wrapping_add(1);
            }
    ...
    fn render_chat(&mut self, frame: &mut Frame, area: Rect) {
        ...
        for msg in &self.messages {
            let lines = match msg {
                ...
                Message::ToolResult(m) => widgets::render_tool_result(m, area.width),
                Message::System(m) => widgets::render_system_message(m),
                Message::UserCommand(m) => widgets::render_user_command_message(m),
                Message::Error(m) => widgets::render_error_message(m, area.width),
                Message::Warning(m) => widgets::render_warning_message(m, area.width),
...
impl TerminalWrapper {
    ...
    pub fn new() -> io::Result<Self> {
        ...
        let viewport_height = height.saturating_sub(1).max(1);
    ...
    pub fn restore(&mut self) -> io::Result<()> {
        disable_raw_mode()?;
        execute!(self.terminal.backend_mut(), DisableBlinking)?;
