...
//!
//! The agent runs as a tokio task on the caller's runtime, communicating
...
pub enum AgentCommand {
    ...
    SetMode(AgentMode),
    /// Save snapshot.
    SaveSnapshot { name: Option<String> },
    /// Shutdown the worker.
    Shutdown,
}
...
impl AgentHandle {
    /// Create a new agent handle and worker future.
    ///
    /// The returned future drives the worker loop — the caller is responsible
    /// for spawning or otherwise driving it (e.g. `tokio::spawn(worker)`).
    pub fn new(
        ...
        let handle = Self { cmd_tx, result_rx };
        let worker = run_worker(config, mode, backend, cmd_rx, result_tx);
        (handle, worker)
    }
    ...
    /// Send a shutdown command and drop the handle.
...
/// The worker loop — processes commands and forwards agent events.
async fn run_worker(
    config: ParameciaConfig,
    mode: AgentMode,
    backend: Option<Arc<dyn Backend>>,
    ...
    let mut agent = match if let Some(backend) = backend {
        Agent::with_backend_options(config, mode, backend, None, None).await
    } else {
        Agent::with_options(config, mode, None, None).await
    } {
        Ok(agent) => {
    ...
    loop {
        ...
        match cmd {
            AgentCommand::Act { prompt } => {
                ...
                let mut act_result: Option<Result<(), paramecia_harness::error::ParameciaError>> =
                    None;
            ...
            AgentCommand::SaveSnapshot { name } => {
                ...
                match agent.save_snapshot(&snapshot_name).await {
                    Ok(path) => {
                        let _ = result_tx
                            .send(AgentResult::SnapshotSaved {
                    ...
                    Err(e) => {
                        let _ = result_tx
                            .send(AgentResult::SnapshotError(e.to_string()))
                            .await;
                    }
                }
            }
