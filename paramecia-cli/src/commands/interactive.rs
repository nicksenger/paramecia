...
//!
//! Uses message-passing concurrency: the agent runs as a tokio task,
...
fn apply_tui_overrides(config: &mut ParameciaConfig) {
...
pub async fn run(
    ...
    tui_args: &TuiArgs,
    _loaded_messages: Option<Vec<paramecia_text::LlmMessage>>,
) -> Result<()> {
    if let Some(controller_path_ref) = controller_path.as_deref().map(Path::new) {
        if !controller_path_ref.exists() {
            return Err(anyhow!(
                "Controller module not found: {}",
                controller_path_ref.display()
            ));
        }
        if !controller_path_ref.is_file() {
            return Err(anyhow!(
                "Controller module path is not a file: {}",
                controller_path_ref.display()
            ));
        }
    }
    ...
    if !tui_args.enabled_tools.is_empty() {
    ...
    // Set context length for token display
    if config.context_length > 0 {
        app.update_tokens(0, config.context_length);
    }
    ...
    // Set up backend — either a controller backend or None for default local inference
    let mut controller_ready_rx: Option<tokio::sync::oneshot::Receiver<Result<(), String>>> = None;
    let backend: Option<Arc<dyn Backend>> = if let Some(controller_path) = controller_path {
        // Create bridge channels
        let (bridge, host_endpoint) = paramecia_bridge::create_bridge(32);
        let (controller_backend, ready_rx) = paramecia_text::ControllerBackend::new(bridge);
        controller_ready_rx = Some(ready_rx);
        let backend: Arc<dyn Backend> = Arc::new(controller_backend);
        // Resolve model/tokenizer from controller config first, then active model provider,
        // then environment fallback. Using providers.first() is incorrect when multiple
        // providers are configured.
        let active_provider_paths = config
            .get_active_model()
            .ok()
            .and_then(|model| config.get_provider_for_model(model).ok())
            .map(|provider| {
                (
                    provider.local_model_path.clone(),
                    provider.local_tokenizer_path.clone(),
                )
            });
        let (provider_model_path, provider_tokenizer_path) =
            active_provider_paths.unwrap_or_default();
        ...
        let model_path = config
            ...
            .or(provider_model_path)
            .or_else(|| std::env::var("PARAMECIA_MODEL_PATH").ok());
        let tokenizer_path = config
            .controller
            .tokenizer_path
            .as_ref()
            .map(|p| p.to_string_lossy().into_owned())
            .or(provider_tokenizer_path)
            .or_else(|| std::env::var("PARAMECIA_TOKENIZER_PATH").ok());
        // Start WASM controller runtime in background thread
        let controller_path = controller_path.clone();
        let paramecia_bridge::ControllerHostEndpoint { user_rx, agent_tx } = host_endpoint;
        let controller_error_tx = agent_tx.clone();
        std::thread::spawn(move || {
            let interactive_endpoint =
                paramecia_bridge::ControllerHostEndpoint { user_rx, agent_tx };
            let options = paramecia_host::HostOptions {
                controller: controller_path,
                model_path,
                tokenizer_path,
                interactive_endpoint: Some(interactive_endpoint),
                quiet: true,
                ..Default::default()
            };
            if let Err(e) = paramecia_host::run_host_cli(options) {
                let msg = format!("Controller runtime error: {e}");
                let _ = controller_error_tx
                    .blocking_send(paramecia_bridge::ControllerAgentEvent::Error(msg.clone()));
                tracing::error!("{msg}");
            }
        });
        Some(backend)
    ...
    let (mut agent_handle, worker) = AgentHandle::new(config.clone(), mode, backend);
    tokio::spawn(worker);

    // Animate loading screen while waiting for agent to initialize
    let session_id;

    // Phase 1: wait for agent worker to be ready
    loop {
        app.tick();
        ...
        if let Some(result) = agent_handle.try_recv() {
            match result {
                AgentResult::Ready { session_id: sid } => {
                    session_id = sid;
                    if controller_ready_rx.is_none() {
                        // Non-controller backend — ready immediately
    ...
    // Phase 2: for controller backends, wait for guest Initialized signal
    if let Some(mut ready_rx) = controller_ready_rx.take() {
        loop {
            app.tick();
            if terminal.draw(&mut app).is_err() {
                return Err(anyhow!("Failed to render loading state"));
            ...
            match ready_rx.try_recv() {
                ...
                Ok(Err(e)) => {
                    ...
                    let _ = terminal.draw(&mut app);
                    time::sleep(Duration::from_secs(2)).await;
                    return Err(anyhow!("Controller error: {}", e));
                }
                ...
                Err(tokio::sync::oneshot::error::TryRecvError::Closed) => {
                    app.set_model_loading(false);
                    app.add_system_message(
                        "Controller disconnected during loading".to_string(),
                        SystemMessageKind::Error,
                    );
                    let _ = terminal.draw(&mut app);
                    time::sleep(Duration::from_secs(2)).await;
                    return Err(anyhow!("Controller disconnected during loading"));
                }
            }
                    ...
                    match handle_app_action(action, &mut app, &agent_handle).await {
                        ...
                        LoopControl::Break => {
                            ...
                            if let Some(session_id) = &app.session_id {
                                println!();
                                println!("To continue this session, run: paramecia tui --continue");
                                println!(
                                    "Or: paramecia tui --resume {}",
...
fn handle_agent_result(app: &mut App, result: AgentResult) {
    match result {
        ...
        AgentResult::Event(event) => {
            ...
            match event {
                AgentEvent::Assistant(e) => {
                    ...
                    if let Some(context_tokens) = e.context_tokens {
                        let monotonic_tokens = context_tokens.max(app.token_context.0);
                        app.update_tokens(monotonic_tokens, app.token_context.1);
                    }
                }
                AgentEvent::ToolCall(e) => {
                    app.add_tool_call(e.tool_name, e.tool_call_id, &e.args);
                    if let Some(context_tokens) = e.context_tokens {
                        let monotonic_tokens = context_tokens.max(app.token_context.0);
                        app.update_tokens(monotonic_tokens, app.token_context.1);
                    }
                ...
                AgentEvent::ContextTokens(e) => {
                    let monotonic_tokens = e.context_tokens.max(app.token_context.0);
                    app.update_tokens(monotonic_tokens, app.token_context.1);
                }
        ...
        AgentResult::SnapshotSaved { path } => {
            app.add_system_message(format!("Snapshot saved: {}", path), SystemMessageKind::Info);
        }
        AgentResult::SnapshotError(e) => {
            app.add_system_message(
...
async fn handle_command(app: &mut App, agent_handle: &AgentHandle, cmd: &str) -> LoopControl {
    ...
    match command.as_deref() {
        ...
        Some("/reload") => {
            match ParameciaConfig::load(None) {
                Ok(new_config) => {
        ...
        Some("/snapshot") => {
            // Extract optional snapshot name
            let snapshot_name = parts.get(1).map(|s| s.to_string());

            app.add_system_message("Saving snapshot...".to_string(), SystemMessageKind::Info);
            let _ = agent_handle
                .send(AgentCommand::SaveSnapshot {
                    name: snapshot_name,
                })
                .await;
            LoopControl::Continue
